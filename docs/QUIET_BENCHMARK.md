...
Generated by `npm run bench:quiet` on 2026-09-08. Checkouts and raw results live under `benchmarks/quiet/` and are not committed; the manifest (`repos.json`), the classification rules (`noise-rules.json`) and any per-repo review labels (`labels/<repo>.json`) are.
