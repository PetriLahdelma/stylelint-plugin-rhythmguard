...
Generated by `npm run bench:quiet` on 2026-09-23. Checkouts and raw results live under `benchmarks/quiet/` and are not committed; the manifest (`repos.json`), the classification rules (`noise-rules.json`) and any per-repo review labels (`labels/<repo>.json`) are.
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| [adminlte](https://github.com/ColorlibHQ/AdminLTE.git) | `12b4b06` | 46 | 0 | 133 | 133 | 0 | 0 | 0% | 191 | installed bootstrap@5.3.8 (7 tokens) |
...
| [calcom](https://github.com/calcom/cal.com.git) | `e91bb0c` | 4 | 203 | 41 | 41 | 0 | 0 | 0% | 61 | fallback (no spacing tokens found) |
| [carbon-styles](https://github.com/carbon-design-system/carbon.git) | `4cc7900` | 284 | 73 | 212 | 212 | 0 | 0 | 0% | 411 | installed @carbon/layout@11.59.0 (26 tokens) |
...
| [ghost-admin](https://github.com/TryGhost/Ghost.git) | `26746d3` | 119 | 202 | 749 | 749 | 0 | 0 | 0% | 1280 | fallback (no spacing tokens found) |
...
| [mittwald-flow](https://github.com/mittwald/flow.git) | `17efdf9` | 130 | 1423 | 0 | 0 | 0 | 0 | 0% | 2 | installed @mittwald/flow-design-tokens@1.2.8 (16 tokens) |
...
| [patternfly](https://github.com/patternfly/patternfly.git) | `b62f051` | 213 | 0 | 14 | 0 | 14 | 0 | 100% | 23 | scanned-css (51 tokens) |
...
| [primer-css](https://github.com/primer/css.git) | `72564a3` | 113 | 0 | 69 | 69 | 0 | 0 | 0% | 116 | installed @primer/primitives@11.10.0 (19 tokens) |
| [primer-react](https://github.com/primer/react.git) | `dc8387f` | 204 | 442 | 11 | 11 | 0 | 0 | 0% | 33 | installed @primer/primitives@11.10.0 (19 tokens) |
...
Paths: `src/scss`. AdminLTE: inherits Bootstrap's $spacer/$spacers unchanged (maintainer, ColorlibHQ/AdminLTE#6124); layout dimensions in _variables.scss are deliberately not scale steps.

Inferred scale: `0, 4, 8, 16, 24, 48` from installed bootstrap@5.3.8.
...
- `src/scss/_accessibility.scss:197` `.75rem`
...
Paths: `scss`. SCSS, v5 (main). Spacing is the $spacer and $spacers map in scss/_variables.scss, read as Sass tokens. Maintainers will not change v5 Sass (twbs/bootstrap#42907).
...
Paths: `scss`. SCSS, the v6 branch the maintainers asked to see (twbs/bootstrap#42907). $spacers is declared as defaults((...), $spacers) in scss/_config.scss, a thirteen-step ladder from 0 to 48px, read since #143.

Inferred scale: `0, 4, 6, 8, 12, 16, 20, 24, 28, 32, 36, 40, 48` from scanned-css (scss/_card.scss, scss/_config.scss).
...
- `scss/_avatar.scss:140` `-2px`
- `scss/_calendar.scss:153` `-.125rem`
- `scss/_calendar.scss:293` `.125rem`
- `scss/_calendar.scss:294` `.125rem`
- `scss/_menu.scss:206` `.125rem`
...
Paths: `packages/styles/scss`. SCSS. Spacing through $spacing-NN tokens and the spacing() function, which @carbon/styles forwards from @carbon/layout; the bench installs that package and measures against its $spacing-01..13 ladder (allowlisted since 3.3.0, issue #87).

Inferred scale: `0, 2, 4, 8, 12, 16, 24, 32, 40, 48, 64, 80, 96, 160` from installed @carbon/layout@11.59.0.
...
- `packages/styles/scss/components/button/_button.scss:280` `1.5px`
- `packages/styles/scss/components/button/_button.scss:441` `13px`
...
Inferred scale: `0, 2, 4, 6, 8, 10, 12, 14, 16, 20, 24, 28, 32, 36, 40, 44, 48, 56, 64, 80, 96, 112, 128, 144, 160, 176, 192, 208, 224, 240, 256, 288, 320, 384` from scanned-css (app/assets/stylesheets/common/base/discourse.scss, app/assets/stylesheets/common/components/design-wizard.scss, app/assets/stylesheets/common/foundation/base.scss).
...
Paths: `packages/styles/src`. SAP Fundamental Styles: spacing comes from SAP theming tokens in @sap-theming/theming-base-content and varies by theme (maintainer, SAP/fundamental-styles#6390); that package has no spacing ladder, values like --sapContent_Margin_M are shorthands.
...
Paths: `packages/components/src`. mittwald Flow: one size scale in px and rem (--size-px--*, --size-rem--*) from @mittwald/flow-design-tokens; components reach it through a component-token layer, so literal scans measure the wrong layer (maintainer, mittwald/flow#3129, see issue #110).

Inferred scale: `0, 1, 2, 4, 8, 16, 24, 32, 64` from installed @mittwald/flow-design-tokens@1.2.8.
...
- `noise:vendored` (4)
  - `src/patternfly/assets/fontawesome/_bordered-pulled.scss:8` `.2em`
  - `src/patternfly/assets/fontawesome/_bordered-pulled.scss:8` `.15em`
  - `src/patternfly/assets/fontawesome/_bordered-pulled.scss:19` `.3em`
  - `src/patternfly/assets/fontawesome/_bordered-pulled.scss:20` `.3em`
...
Paths: `src`. Primer CSS: spacing ladder is --base-size-* from @primer/primitives (maintainer, primer/css#3166 points at src/tokens/functional/spacing/space.json5).

Inferred scale: `0, 2, 4, 6, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 64, 80, 96, 112, 128` from installed @primer/primitives@11.10.0.
...
- `src/buttons/button.scss:8` `5px`
- `src/buttons/button.scss:317` `3px`
- `src/buttons/misc.scss:90` `5px`

### primer-react

Paths: `packages/react/src`. Primer React: same tokens as Primer CSS, from @primer/primitives (maintainer, primer/react#8387).
...
- `packages/react/src/Timeline/Timeline.module.css:108` `-15px`
- `packages/react/src/internal/components/TextInputWrapper.module.css:103` `3px`
- `packages/react/src/internal/components/TextInputWrapper.module.css:106` `3px`
...
Paths: `packages/main/src/themes`. UI5 Web Components: spacing derives from SAP Fiori theming tokens (--sap*) in @sap-theming/theming-base-content plus per-component --_ui5_* tokens; no repo-local scale by design (maintainer, UI5/webcomponents#14027).
...
Paths: `src/client/theme-default/styles`. VitePress default theme: plain CSS with --vp-* tokens. Maintainer (vuejs/vitepress#5423): no spacing scale, none planned; measured against the fallback for information only.
