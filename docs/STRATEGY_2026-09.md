...
Status 2026-09-06: generator shipped (`npm run bench:state-of-spacing`, output in [`STATE_OF_SPACING.md`](./STATE_OF_SPACING.md) and `docs/state-of-spacing/<edition>.{md,json}`). Manifest widened to 57 repositories the same day and the edition regenerated over all of them. Audit issues drafted per repository by `npm run bench:outreach` and posted where the repository accepts free-form issues; repositories with blank issues switched off, issues disabled, archived, or hosted off GitHub are listed in [`outreach/embed-log.md`](./outreach/embed-log.md) for a person to handle through their channel. Publication of the edition as an article waits for those and for the first responses.
