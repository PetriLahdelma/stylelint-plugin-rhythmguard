# Spacing scale audit: 0 literal spacing values off your own token scale
...
**What was run.** `npx rhythmguard audit . --scale auto` at `e65535e` over `packages/skeleton/src`. Hairlines of one pixel or less, percentages, and generated or test paths are excluded. Anyone can reproduce it in a checkout of that commit.

**What it found.** Scale `0, 2, 4, 6, 8, 10, 12, 14, 16, 20, 24, 28, 32, 36, 40, 44, 48, 56, 64, 80, 96, 112, 128, 144, 160, 176, 192, 208, 224, 240, 256, 288, 320, 384` inferred from your own spacing tokens in the stylesheets. 0 literal spacing values are off that scale.
