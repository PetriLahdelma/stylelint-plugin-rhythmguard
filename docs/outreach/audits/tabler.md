# Spacing scale audit: 12 literal spacing values off your own token scale
...
**What was run.** `npx rhythmguard audit . --scale auto` at `ac001f8` over `core/scss`, `src/scss`. Hairlines of one pixel or less, percentages, and generated or test paths are excluded. Anyone can reproduce it in a checkout of that commit.

**What it found.** Scale `0, 0.64, 4, 8, 16, 24, 32, 40, 48, 64, 80, 96, 112, 128` inferred from your own spacing tokens in the stylesheets. 12 literal spacing values are off that scale.

- Values: `-90px` ×3, `85px` ×3, `-2px` ×1, `0.1em` ×1, `0.1rem` ×1
- Properties: `transform` ×7, `padding` ×2, `margin` ×1, `margin-inline-end` ×1, `margin-top` ×1
