# Spacing scale audit: 57 literal spacing values off your own token scale
...
**What was run.** `npx rhythmguard audit . --scale auto` at `c6fad86` over `public/sass`. Hairlines of one pixel or less, percentages, and generated or test paths are excluded. Anyone can reproduce it in a checkout of that commit.

**What it found.** Scale `0, 2, 4, 8, 16, 24, 32` inferred from your own spacing tokens in the stylesheets. 57 literal spacing values are off that scale.

- Values: `10px` ×9, `15px` ×9, `5px` ×7, `6px` ×6, `0.45em` ×4
- Properties: `padding` ×34, `padding-left` ×8, `margin-top` ×4, `margin-right` ×3, `padding-top` ×3
