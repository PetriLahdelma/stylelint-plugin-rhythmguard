# Spacing scale audit: 9 literal spacing values off your own token scale
...
**What was run.** `npx rhythmguard audit . --scale auto` at `c9d6914` over `sass`. Hairlines of one pixel or less, percentages, and generated or test paths are excluded. Anyone can reproduce it in a checkout of that commit.

**What it found.** Scale `0, 4, 8, 12, 16, 24, 48` inferred from your own spacing tokens in the stylesheets. 9 literal spacing values are off that scale.

- Values: `10px` ×3, `15px` ×2, `14px` ×1, `20px` ×1, `5px` ×1
- Properties: `padding` ×5, `padding-left` ×2, `margin` ×1, `margin-top` ×1
